...
§2.3 的产出 `docs/design/<feature>/detailed-design.md` 按以下结构组织——§2.3.1 完整性 6 条与 §2.3.2 函数论证嵌入到 §4 模块细化的每个函数子节，§5 完整性自检 checklist 作为提交前 sanity gate：
...
2. **细化文档结构就位**（§2.3.3）：detailed-design.md 含 §1 范围 / §2 复用 / §3 错误处理 / §4 模块细化 / §5 完整性自检 checklist 各段
...
> **本章导读**：§3.1 功能 / §3.2 接口（基础）/ §3.4 书写原则 是任何项目都要读的通用基础；§3.3 并发规约仅多线程 / 并行项目需读；分布式接口契约扩展（七项契约 + 反模式）独立成**附录 A**，仅涉及跨进程 / 跨机器通信的项目需读。
...
涉及跨进程 / 跨机器通信的接口规约，需在 §3.2 基础模板之上追加本附录定义的七项分布式契约。**不适用的维度也要显式写出 "N/A + 理由"，禁止省略。**
