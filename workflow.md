...
- **禁止**删除或注释掉失败的测试用例
- **禁止**修复完成而不补回归测试用例（详见 §5 第 4 步 / §5.1 第六部分）
- **禁止**修复完成而不同步契约文档（"涉及契约变更"的判定与同步清单见 §5.1 第八部分）
...
> **本章导读**：§2.1 调研 → §2.2 架构 → §2.3 细化 → §2.4 审查。架构层（§2.2）和细化层（§2.3）各有一个"正确性论证"副节（§2.2.1 / §2.3.1 / §2.3.2）——这是把"正确性有保证"从口号变成可判定标准的关键。§2.4 审查 5 条全部对应到 §2.2.1 / §2.3.1 / §2.3.2 的产出。
>
...
产出细化文档（`docs/design/<feature>/detailed-design.md`，整体结构见 §2.3.3 模板），内容包括：
1. 逐模块列出需要实现的函数：函数签名、功能描述、调用关系
2. 与已有代码的复用点
3. 错误处理策略
4. **数据结构定义**（如本细化引入新类型 / 修改既有类型；按 §3.1 数据结构规约格式写入模板 §4）
5. **完整性 6 条全过**（见 §2.3.1）
6. **每个非平凡函数的正确性论证**（见 §2.3.2）
...
6. **显式假设链**：前置条件、上游事实不能默认带过来。每步显式列出本步依赖的事实——避免"想当然"。**区别于 §2.2.1 关键假设**：本条是函数实现层的逐步事实链（"为什么这一步能从上一步推出"），§2.2.1 是项目级外部前提（"整个架构假设输入有序"）

> **6 条与 §3.2 功能规约的关系**：§3.2 规约写 "**是什么**"（pre / post / invariant），§2.3.1 完整性是写细化时 "**怎么演绎**" 的格式要求。两者互补，不替代。
...
**示例 2（non-trivial，必填）**：假设 §2.2.1 关键假设含 `H1: db transaction 隔离级别 ≥ READ_COMMITTED`、`H2: log.write 失败不影响主流程`。
...
§2.3 的产出 `docs/design/<feature>/detailed-design.md` 按以下结构组织——§2.3.1 完整性 6 条与 §2.3.2 函数论证嵌入到 §5 模块细化的每个函数子节，§6 完整性自检 checklist 作为提交前 sanity gate：
...
## 1. 范围
- 本细化覆盖的函数 / 模块清单
- 与架构文档 §2.2 的对应（实现哪些 goal / 涉及哪些模块）
- 与子计划 expectations.md 的对应（仅 §4.5 v2 子计划）
...
- **实现思路**（按 §2.3.1 完整性 6 条编写：推导连续 / 分支覆盖 / 退出覆盖 / callee 引用 / 循环刻画 / 显式假设链）
- **正确性论证**（按 §2.3.2 格式；trivial 函数标 "trivial + 理由"）
...
- **超过 ~800 行**：按 §6.4 拆为子计划（`docs/design/<feature>/subplans/<subplan-id>.md`）；或按模块拆（`docs/design/<feature>/modules/<module>.md`，主文档保留 §1-§3 + §4 模块清单跳转 + §5 全局自检）
...
1. **架构正确性论证就位**（§2.2.1）：goal → 模块映射明确、模块协作论证通过、关键假设显式列出
2. **细化文档结构就位**（§2.3.3）：detailed-design.md 含 §1 范围 / §2 复用 / §3 错误处理 / §4 数据结构定义 / §5 模块细化 / §6 完整性自检 checklist 各段
3. **细化完整性 6 条全过**（§2.3.1）：推导连续 / 分支覆盖 / 退出覆盖 / callee 契约引用 / 循环刻画 / 显式假设链
4. **函数正确性论证就位**（§2.3.2）：所有非平凡函数都有论证段；trivial 函数标注 "trivial + 理由"
...
> - §2.3.2 "函数正确性论证"是**为什么对**（为什么本实现能在 pre 下推出 post）—— 规约的陪伴证据
> - §4.5 v2 "expectations" 是**对照表**（事前从契约文档抽出 vs 事后从代码独立审）—— 不替代规约或论证
...
> **与 §2.2.1 架构正确性论证的关系**：跨模块 / 跨线程不变量在 §2.2.1 "模块级 invariant 列表" 段列出（§2.2.1 已对多模块项目硬性要求列出本清单，不在"关键假设"段——那里是外部输入前提，不是模块共享状态）。本节 Rely-Guarantee 是该不变量在并发场景下的具体责任分配——架构论证负责"为什么这些不变量足够"，本节负责"每个线程承担哪部分"。两者配套，不替代。
...
> **§2.3.2 函数论证按子计划增量产出**：v2 子计划要求其引入 / 修改的所有函数都满足 §2.3.2（非平凡函数必有论证段）；已有但本子计划未触及的函数不需要回填论证。**例外（需回填）**：本子计划改动了 callee / caller 的签名或行为契约时，受影响链路上的所有函数都需要更新论证——具体判定为"该函数的论证段引用的 pre / post / callee 契约因本子计划变更"。
...
> **§7 不变量契约与 §2.2.1 / §2.3.2 的关系**：v2 §7 是**契约对照**（事前从契约文档抽出 vs 事后从代码独立审），不替代 §2.2.1 跨模块不变量列表或 §2.3.2 函数正确性论证里的不变量推导。三者职能：
> - §2.2.1 关键假设：**架构层**——本子计划依赖的所有外部假设
> - §2.3.2 函数论证：**实现层**——每个函数为什么能维持声明的 invariant
> - §4.5 v2 §7：**对照层**——双验证机制，确认契约里声明的 invariant 真的在代码里有 property-based 测试覆盖
...
> 后续若发现契约 bug 触发回归修复，按 §5 流程走，并按 §5.1 第八部分"文档更新清单"同步 `expectations.md`。
...
2. **分类处理 + 修正方案文档**：
   ...
   - **算法内部逻辑错误**（实现偏离正确算法、边界处理遗漏等）— 完成根因分析并产出修正方案文档（§5.1 八部分），经确认后直接修复
   - **接口或架构层面问题**（接口不合理、模块划分缺陷、新增功能）— 先完成根因分析（§5.1），再回到设计阶段（§2）调整设计文档，最后按实现流程执行
3. **举一反三** — 梳理是否存在其他同类问题，反思问题产生的原因；同类风险点写入 §5.1 第六部分"新增用例"（显而易见错误也要做这一步，但可直接补测试用例，无须写入 §5.1 清单）
4. **测试同步**（回归 + 新增）— 按 §5.1 第六部分"测试用例清单"落实：(a) 第一部分最小复现用例固化为**回归测试**（必做）；(b) 第六部分列出的新增测试用例落实到 test suite；(c) 重新运行所有已有模块测试，确认无回归。**回归用例未补不算修复完成**（对应 §1.2 操作禁令）
5. **代码同步** — 按 §5.1 第七部分"代码更新清单"逐项核对：所有计划改动都已实施、无遗漏 / 无误改；回填 commit hash
6. **文档同步** — 按 §5.1 第八部分"文档更新清单"逐项更新所列文档：契约文档（research / design / spec）、模块 README、若该模块属于带 `expectations.md` 的子计划还需同步 `expectations.md`。**文档未同步不算修复完成**（对应 §1.2 操作禁令）

> **显而易见错误的简化路径**：步骤 4-6 假定 §5.1 修正方案文档存在（含第六/七/八部分清单）。对于**显而易见的错误**（笔误、off-by-one 等局部问题，按 step 2 第一类处理），§5.1 文档非必须，本流程简化为：(a) 步骤 4 仍须补回归用例 + 跑测试（必做，不放松）；(b) 步骤 5 代码改动直接对照 commit diff 核对（不写清单）；(c) 步骤 6 文档同步——若改动不触及任何契约可跳过，否则按 §5.1 第八部分必填情况判定。
...
类似 §2.3.2 函数正确性论证，针对**修复方案本身**论证三点：
...
修复前列出计划改动的所有文件与位置（与第四部分修复方案对应），修复完成后回填实际 commit hash。本清单与 §5 第 5 步"代码同步"配套使用，确保所有计划改动都已实施、无遗漏 / 无误改。
...
**bug fix 不在本判定流程**：bug fix 走 §5 流程，按 §5.1 第八部分判定是否涉及契约。涉及契约的 bug fix 若发现根因是契约本身错误，按 §5 同步修订契约文档；若发现根因是契约不完整（应当存在但缺失的子计划），可能反推到主流程步骤 2 补开子计划再继续。
...
- **模块私有类型** → 写在该模块的 detailed-design.md 数据结构节（§2.3.3 模板 §4），或独立 `docs/design/<feature>/types.md`
...
Bug 修复（§5 主流程 6 步，§5.1 修正方案文档 8 部分）：
  发现错误     → 修正方案文档(§5.1 八部分: 现象/复现 + 根因 + 参考对照
                  + 修复方案 + 正确性论证 + 测试用例 + 代码更新 + 文档更新)
                  → [确认] → fix 分类
  局部错误     → 直接修复 + 测试同步 + 代码同步 + 文档同步
  算法内部错误 → 修正方案文档 → [确认] → 修复 + 测试同步 + 代码同步 + 文档同步
                 ↑ 算法类 bug 必须对照参考实现
  接口/架构问题 → 修正方案文档 → [确认] → 更新设计(§2) → 按实现流程执行
                                  + 测试同步 + 代码同步 + 文档同步
