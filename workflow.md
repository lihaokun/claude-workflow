...
- **关键假设显式**：本架构能实现目标依赖的所有**隐含前提**必须显式列出。典型如"假设输入有序"、"假设并发度 ≤ N"、"假设上游不重复发送"等。**未声明的假设 = 未来的 bug 源**
...
模块协作论证：
  G1 要求 <X>。模块 A 的功能规约 ensures <Y>，模块 C 的功能规约 ensures <Z>。
  Y ∧ Z ⇒ X，故 G1 成立。
  （非算法类项目可用自然语言论证）
...
5. **循环必填**：任何循环必须有不变量描述 + 终止性论证。**算法 / 验证类项目**硬性要求不变量；**非算法类项目**至少给"为何终止"的非形式论证（如"列表长度有限，每轮减 1"）
6. **显式假设链**：前置条件、上游事实不能默认带过来。每步显式列出本步依赖的事实——避免"想当然"

> **6 条与 §3.1 功能规约的关系**：§3.1 规约写 "**是什么**"（pre / post / invariant），§2.3.1 完整性是写细化时 "**怎么演绎**" 的格式要求。两者互补，不替代。

> **算法 / 形式化验证类项目特化**：可在 6 条基础上引入 Hoare-logic SP reasoning、循环不变量三性质（ESTABLISHMENT / PRESERVATION / INTERIOR）等形式化层。参见 proof-drafter 项目 `docs/design/architecture.md §3.2.3` 作为示例（其 SP Reasoning 7 项 completeness 即本节 6 条的形式化超集）。

#### 2.3.2 函数正确性论证

每个**非平凡**函数必须显式说明"为什么这个实现能满足声明的 pre / post"。这是 §3.1 功能规约的**陪伴论证**：规约说"是什么"，论证说"为什么对"。
...
前置：rows 非空；rows 元素 schema 与 table 匹配
后置：所有 rows 已插入 table 或 InsertResult.failed 非空；table.invariant 仍成立

正确性论证：
  - 前置: rows 非空、schema 匹配（pre 保证）
  - 论证：
    - 步骤 1: 批量校验 schema（complete by §3.1 schema 规约）→ 校验前 invariant
    - 步骤 2: 开启 transaction → 隔离级别 ≥ READ_COMMITTED（来自 §架构关键假设 H1）
    - 步骤 3: 循环逐行 INSERT
      - 不变量：已尝试行数 i ≤ len(rows)；成功行计入 succeeded；失败行计入 failed
      - 终止：i 单调递增，上界 len(rows) 有限
    - 步骤 4: commit → 成功行落盘；rollback on critical error → 全部撤回（来自 callee tx.commit 契约 ensures）
    - 最终：要么所有 rows 已插入（succeeded == rows），要么部分插入 + failed 非空 ⇒ post
  - 后置: post 成立
  - 副作用论证: 仅 table 行变更（INSERT 语义）；transaction commit 是唯一持久化点；其他副作用（如审计日志）由 callee log.write 显式负责，不在本函数列
```
...
2. **细化完整性 6 条全过**（§2.3.1）：推导连续 / 分支覆盖 / 退出覆盖 / callee 契约引用 / 循环必填 / 显式假设链
3. **函数正确性论证就位**（§2.3.2）：所有非平凡函数都有论证段；trivial 函数标注 "trivial + 理由"
4. 细化阶段每个模块的函数设计满足该模块的功能规约和接口规约
5. 文档无歧义，可直接用于开发实现
...
涉及跨进程/跨机器通信时，**必须**追加以下七项分布式契约。不适用的维度也要显式写出 "N/A + 理由"，禁止省略（术语见 §3.4）：
...
3. 重试与幂等：
   ...
   - 退避策略：<指数退避 / 固定间隔 / 是否含 jitter>
   - 最大次数：<...>
   - 接口幂等性：<幂等 / 非幂等>
   - 幂等键（非幂等接口若允许重试，必须定义）：<如何构造，跨重试如何保持一致>
...
5. 失败模式：
   - 对端崩溃时的行为：<区分崩溃故障 / fail-stop 与拜占庭故障；指明本接口假设的故障模型>
   - 网络分区时的行为：<是否可能 split-brain；分区期间读写策略>
   - 慢响应（活着但响应缓慢）时的行为：<...>
...
**反模式警示（分布式接口）**：未声明 ≠ 行为安全。下列五条都是"沉默 = 默认值咬你"的典型场景。
...
> **与 §2.2.1 架构正确性论证的关系**：跨模块 / 跨线程不变量在 §2.2.1 "关键假设" 段列出，本节 Rely-Guarantee 是该不变量在并发场景下的具体责任分配；架构论证负责"为什么这些不变量足够"，本节负责"每个线程承担哪部分"。

### 3.4 分布式术语表

接口规约中使用的分布式术语，统一定义如下，避免每个项目重复解释或各自表述。术语顺序对齐 §3.2 七项分布式契约。

**连接**
- **持久连接**：连接建立后跨多个请求复用，关闭时机由应用控制
- **短连接**：每个请求独立建立和关闭连接
- **连接池**：连接预先建立、按需借出归还，用以控制并发连接数和摊销握手开销

**重试与退避**
- **指数退避（exponential backoff）**：第 n 次重试（n ≥ 1）间隔为 base × 2^(n−1)，即首次重试等 base，第二次 2×base，依此类推
- **jitter**：在退避时间上加随机扰动，避免大量客户端在同一时刻重试造成尖峰

**幂等**
- **幂等接口**：同一请求重复调用 N 次（N ≥ 1）的效果与调用 1 次相同
- **幂等键（idempotency key）**：调用方生成的唯一标识，被调用方据此识别并去重重复请求

**交付语义**
- **at-most-once**：消息至多投递一次，可能丢失，不会重复
- **at-least-once**：消息至少投递一次，不会丢失，可能重复——调用方须配合幂等保证正确性
- **exactly-once**：严格的 exactly-once 投递在异步网络中不可达；工程上以 at-least-once + 幂等去重 实现等价的 effective-once 效果

**顺序保证**
- **FIFO**：同一发送方的消息按发送顺序到达接收方
- **因果序（causal order）**：有 happens-before 关系的消息按因果顺序到达，无因果关系的消息可任意排
- **全序（total order）**：所有参与方观察到的消息顺序一致
- **无序**：不提供任何顺序保证，消息可乱序到达
- **happens-before**：因果先后关系，由程序序、send→receive 同步、显式同步原语的传递闭包共同构成（因果序 / 全序的基础概念，§3.3 顺序约束亦使用）

**失败模型**
- **网络分区（network partition）**：节点间无法通信但各自仍存活，可能形成多个互相不可见的子集
- **split-brain**：网络分区下多个子集都自认为是主，导致状态分裂
- **崩溃故障 / fail-stop**：节点要么正常工作要么停止，不会产生错误响应

**状态**
- **无状态**：被调用方不保存调用方的会话信息，每个请求自包含
- **有会话**：被调用方保存跨请求的会话状态，需要会话标识关联

**流控**
- **背压（backpressure）**：下游处理不过来时反向通知上游减速或暂停
- **流控（flow control）**：通过窗口、令牌、限速等机制约束发送速率
...
> **§2.3.2 函数论证按子计划增量产出**：v2 子计划要求其引入 / 修改的所有函数都满足 §2.3.2（非平凡函数必有论证段）；已有但本子计划未触及的函数不需要回填论证（除非该函数本身是契约变更的间接影响范围）。
