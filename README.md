...
包含设计三阶段流程、半形式化规约书写规范、分层测试策略、根因分析驱动的 bug 修复流程、契约对照审核 v2（消除自审 bias 的结构化双验证），以及 Claude 行为准则。适用于算法密集型项目（如数学库、编译器、形式化验证工具等），也可作为通用项目的开发规范基础。
...
- **workflow.md**：如果项目不涉及跨进程/跨机器通信，可忽略 §3.2 分布式契约段和 §3.4 术语表。如果项目不涉及并发，可忽略 §3.3 并发规约。如果不是算法类项目，§5.1 的参考实现对照可简化
